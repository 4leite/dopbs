...
(function($){
    $.fn.DOPBookingSystemPRO = function(options){
        var Data = {'AddLastHourToTotalPrice': true,
                    ...
                    'PaymentArrivalSuccessInstantBooking': 'Your request has been successfully submitted. We will send you a confirmation email with payment details. Please note your booking is not confirmed until payment is received.',
...
//                        HTML.push('                <label for="DOPBookingSystemPRO_CheckIn'+ID+'">'+CheckInLabel+'</label>');
                        HTML.push('                <label for="DOPBookingSystemPRO_CheckIn'+ID+'">Tour Date</label>');
...
//                        HTML.push('                    <label for="DOPBookingSystemPRO_NoItems'+ID+'">'+NoItemsLabel+'</label>');
                        HTML.push('                    <label for="DOPBookingSystemPRO_NoItems'+ID+'">Number Of People</label>');
                    ...
                    calculateDaysPrice:function(){
                        ...
                        if (ciDay.split('-').length == 3 && coDay.split('-').length == 3 && Schedule != {}){
                            ...
                            if (!available){
                                ...
                                if (Schedule[$('#DOPBookingSystemPRO_CheckIn'+ID).val()]['tour_type'] != null ) {
                                                    $('#DOPBookingSystemPRO_FormField5_14').attr('disabled', 'disabled');
                                                    $('#DOPBookingSystemPRO_FormField5_14').val(Schedule[$('#DOPBookingSystemPRO_CheckIn'+ID).val()]['tour_type']);
                                   } else {
                                   $('#DOPBookingSystemPRO_FormField5_14').val(7);
                                   $('#DOPBookingSystemPRO_FormField5_14').removeAttr('disabled');
                                   }
                    ...
                    book:function(){
                        if (methods.validForm()){
                            ...
                            $.post(ajaxURL, {action:'dopbsp_book_request', 
                                ...
                                methods.resetSidebar();
PaymentArrivalSuccessInstantBooking = PaymentArrivalSuccess = 'Your request has been successfully submitted. We will send you a confirmation email with payment details. Please note your booking is not confirmed until payment is received.'
